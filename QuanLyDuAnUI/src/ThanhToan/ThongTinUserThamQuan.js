...
const PassengerForm = () => {
  ...
  const [formData, setFormData] = useState({
    nguoiLon: Array(in4.sl.sl_NguoiLon).fill({ ten: '', CCCD: '', gioitinh: '', namsinh: '' }),
    treEm: Array(in4.sl.sl_TreEm).fill({ ten: '', gioitinh: '', namsinh: '' }),
    sdt: '',
    email: '',
    chkAgreeTerms: false,
  });
  ...
  const handleChange = (index, field, value, type) => {
    if (type === 'treEm' && field === 'namsinh' && !isAgeValid(value, 0)) {
        alert("Ngày sinh không hợp lệ.");
        return;
      }
    if (type === 'nguoiLon' && field === 'namsinh' && !isAgeValid(value)) {
  ...
  const handleSubmit = (e) => {
    ...
    const dl={
      nguoiThan:[...formData.nguoiLon.map(({ ten, namsinh }) => ({ten, namsinh })), 
        ...formData.treEm.map(({ ten, namsinh }) => ({ ten, namsinh }))],
      sdt: formData.sdt,
      email: formData.email,
    }
    console.log("dldad",dl);
    localStorage.setItem('thongTinChung', JSON.stringify(dl));
    navigate('/thanhtoan');
  };
      ...
      <form onSubmit={handleSubmit}>
            ...
            <div className="row" key={i}>
              <div className="col p-3 text-dark">
                ...
                <input
                  ...
                  value={formData.nguoiLon[i].ten}
                  onChange={(e) => handleChange(i, 'ten', e.target.value, 'nguoiLon')}
                  required
                />
              ...
              <div className="col p-3 text-dark">
                ...
                <input
                  type="date"
                  className="form-control p-2 m-1"
                  value={formData.nguoiLon[i].namsinh}
                  onChange={(e) => handleChange(i, 'namsinh', e.target.value, 'nguoiLon')}
                  required
                />
              </div>
            ...
            <div className="row" key={j}>
              ...
              <div className="col p-3 text-dark">
                ...
                <input
                  ...
                  className="form-control p-2 m-1"
                  placeholder="Vi du: Nguyen Van An"
                  value={formData.treEm[j].ten}
                  onChange={(e) => handleChange(j, 'ten', e.target.value, 'treEm')}
                  required
                />
              </div>
              <div className="col p-3 text-dark">
              ...
              <div className="col p-3 text-dark">
                ...
                <input
                  type="date"
                  className="form-control p-2 m-1"
                  value={formData.treEm[j].namsinh}
                  onChange={(e) => handleChange(j, 'namsinh', e.target.value, 'treEm')}
                  required
                />
              </div>
            </div>
        ...
        <div className="container2 text-dark">
          ...
          <div className="d-flex p-2 m-1">
            ...
            <div style={{ width: '60%' }}>
              <div className="d-flex">
                <div className="me-auto">{in4.sl.sl_NguoiLon} người lớn</div>
                <div className="me-auto">{in4.sl.sl_TreEm} Trẻ em</div>
              </div>
              <hr />
              <div className="d-flex">
                <div className="me-auto">Tổng Cộng:</div>
                <div className="ms-auto">{new Intl.NumberFormat().format(in4.tong)} VNĐ</div>
              </div>
            </div>
          </div>
        </div>
