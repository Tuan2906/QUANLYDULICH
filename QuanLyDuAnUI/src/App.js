...
const Main = ({user}) => {
  ...
  return (
    ...
    {console.log("user",user)}
      ...
      {!isLoginPageNV &&!isLoginPage && user===null && <Header cart={cart} countCart={countCart} 
      page={page} setPage={setPage} loadingCart={loadingCart} setLoadingCart={setLoadingCart}/>
      }
      {
        !isLoginPageNV && <Container>
        <Routes>
          <Route path="/Home" element={<Home />} />
          <Route path="/Home/detail/:post_id" element={<PostDetail update={update} setUpdate={setUpdate} setCountCard={setCountCart} />} />
...
function App() {
  const [user, dispatch] = useReducer(MyUserReducer, localStorage.getItem("user") || null);
