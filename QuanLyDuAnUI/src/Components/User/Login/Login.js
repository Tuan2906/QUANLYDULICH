  ...
  const handleSubmit = async (e) => {
    ...
    if(user.avatar) {
      ...
      try {
        ...
        alert("Tạo tài khoảng thành công, Vui lòng nhập lại username và password")
        setIsSignIn(!isSignIn);
      }
    }
    else {
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="avatar-container">
        <div className="avatar">
          {image ? <img src={image} alt="Avatar" /> : <i className="fas fa-user fa-3x"></i>}
...
const SignInForm = () => {
  ...
  return (
          <form onSubmit={loginUser}>
            ...
            {loading ? <MySpinner  animation="grow" size="sm"/> : <button type='submit'>Sign In</button>}
            </form>
  );
};
...
const TogglePanel = ({ isSignIn, onToggle }) => {
  return (
    <div className="toggle">
      <div className={`toggle-panel ${isSignIn ? 'toggle-left' : 'toggle-right'}`}>
        {isSignIn ? (
          <>
            <h1>Welcome Back!</h1>
