...
import {FaMapMarkerAlt,FaPlaneDeparture,FaCalendarAlt,FaStar  ,FaUtensils,FaUsers,FaMap,FaShoppingCart,FaRegCommentDots   } from 'react-icons/fa';
...
const addCart = async () => {
  ...
  return (
    ...
    <Container>
        ...
        <Carousel
         ...
         >
          {post.pic.map((pic, index) => (
            <div key={index}>
                <img src={pic.picture} alt={post.title}  style={{ maxWidth: '100%', height: '80vh' }}/>
            </div>
          ))}
        ...
        <Row >
          <h1 style={{textAlign:"center", fontSize:90}}>{post.title}</h1>
          <div className="journey-info">
            <Col md={4} style={{textAlign:'center', fontSize:30}}>
              <FaCalendarAlt /><h2 >Thời gian</h2> <div> {changeVi(post.journey.ngayDi)} - {changeVi(post.journey.ngayDen)}</div>
            </Col>
            <Col md={4} style={{textAlign:'center', fontSize:30}}>
            <FaMap /><h2 >Hành trình</h2> <div> {post.journey.id_tuyenDuong.id_noiDi.diaChi} - { post.journey.id_tuyenDuong.id_noiDen.diaChi}</div> 
            </Col>
            <Col md={4} style={{textAlign:'center', fontSize:30}}>
             <FaPlaneDeparture/><h2 >Phương tiện</h2> <div> {post.journey.id_PhuongTien.loai}</div>
            </Col>
          </div>
          </Row>
          <Row>
          ...
          <FaMapMarkerAlt/> <h2>Địa điểm dừng chân</h2>
            ...
            <Col md={4} style={{textAlign:'center', fontSize:30}}>
             <FaUtensils/><h2>Ẩm thực</h2><div>Nhiều loại ẩm thực</div>
            </Col>
            <Col md={4} style={{textAlign:'center', fontSize:30}}>
            <FaUsers /><h2>Tags</h2><p> {post.tags.map(tag => tag.name).join(', ')}</p>
            </Col>
          </Row>
              ...
              <Row style={{textAlign:'center', marginTop:50}}>
                ...
                {post!=null && <CalendarWithModal  dateData ={post.journey.ngayDi} post={post}/>}
                </Col>
              </Row>
