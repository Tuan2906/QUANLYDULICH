...
import { FaShoppingCart ,FaSignOutAlt,FaMotorcycle,FaEdit, FaTrashAlt} from 'react-icons/fa'; // Import icon giỏ hàng từ react-icons
...
const Header = ({cart, countCart,page,setPage,loadingCart,setLoadingCart}) => {
 ...
 "count": 12,
  ...
  return (
    <>
    <div className="header finisher-header" style={{ width: '100%', height: '100px'}}>
     <Navbar bg="transparent" variant="dark" expand="lg">
      <Container>
        ...
        <StyledNavbar className='me-auto'>
          ...
          <Nav>
              {user_cur.current===null? <MySpinner  animation="grow" size="sm"/>:
              <>
              <Image src={user_cur.current.avatar} roundedCircle style={{ width: '30px', marginRight: '10px' }} />
            <span className="text-light">{user_cur.current.username}</span>
              </>
            }
            </Nav>
            <div href="#cart" style={{ cursor:'pointer', marginLeft: 20, marginRight: 20, position: 'relative' }}>
            <FaShoppingCart size={20} color="white" onClick={() => setLgShow(true)} />
            {countCart > 0 && (
              <span
                style={{
                  position: 'absolute',
    ...
    <Modal
      ...
      >
        ...
        {
          countCart == 0? <h1>không có gì trong giỏ hàng</h1>: 
          <Modal.Body className="modal-body-scroll" onScroll={(e)=>handleScroll(e)}>
          <div>
            {!loading ? <MySpinner /> : cart.map((item, index) => (
